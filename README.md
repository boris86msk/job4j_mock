...
#### Docker.
Мы используем docker-compose для
управления многоконтейнерным приложением, что позволяет легко
развернуть и управлять сервисами.

Перед тем как начать процесс контейнеризации, убедитесь, что у вас установлены следующие компоненты:

• Docker (https://docs.docker.com/get-docker/) (версии 3.9)

• Docker Compose (https://docs.docker.com/compose/install/) (версии 1.27 или выше)

Перейдите в директорию проекта:
```
cd job4j_mock
```
В корне проекта находится файл docker-compose.yml. Для запуска процесса выполните следующую команду:
```
docker-compose up
```
(для запуска в фоновом режиме добавьте флаг -d)
после успешного завершения приложение должно отвечать на    http://localhost:8081
Чтобы остановить приложение, выполните команду:
```
docker-compose down
```
Если вы хотите посмотреть логи работы контейнеров, выполните команду:
```
docker-compose logs
```
Для удаления всех остановленных контейнеров и неиспользуемых образов можно использовать команду:
```
  docker system prune
```
